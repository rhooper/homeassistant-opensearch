...
os_library_logger = logging.getLogger("opensearch")
os_library_logger.name = "custom_components.opensearch.library"

# if the logger is already set up, don't change the level
if LOGGER.level == logging.NOTSET:
    LOGGER.setLevel(logging.INFO)

if os_library_logger.level == logging.NOTSET:
    os_library_logger.setLevel(logging.ERROR)
...
def have_child(name: str) -> logging.Logger:
    ...
    sanitized_name = "".join(
        [c if c.isalnum() else "" for c in name.replace(" ", "_").lower()]
    )
...
def log_enter_exit_info(func: Callable) -> Callable:
    ...
    def decorated_func(*args, **kwargs):  # noqa: ANN202
        logger = (
            getattr(args[0], "_logger", LOGGER) if args and len(args) > 0 else LOGGER
        )
...
def async_log_enter_exit_info(func: Callable[..., Coroutine]):  # noqa: ANN201
    ...
    async def decorated_func(*args, **kwargs):  # noqa: ANN202
        ...
        return await call_and_log_enter_exit(
            func, logger, logging.INFO, *args, **kwargs
        )
...
def async_log_enter_exit_debug(func: Callable[..., Coroutine]):  # noqa: ANN201
    ...
    async def decorated_func(*args, **kwargs):  # noqa: ANN202
        ...
        return await async_call_and_log_enter_exit(
            func, logger, logging.DEBUG, *args, **kwargs
        )
